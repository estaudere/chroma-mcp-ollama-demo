...
@mcp.tool()
async def chroma_add_memory(
    text: str,
    category: Optional[str] = None
) -> str:
    """Add a new memory to the Chroma database.
    
    Args:
        text: The text of the memory
        category: The category of the memory (optional, defaults to "general")
    """
    ...
    try:
        ...
        return f"Successfully added memory with category {category}"
    except Exception as e:
        raise Exception(f"Failed to add memory: {str(e)}") from e

@mcp.tool()
async def chroma_get_memories(
    category: Optional[str] = None,
